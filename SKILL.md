...
┌────────────────────────────────────────────┐
│  安全验证分支：引导用户完成身份校验         │
│  返回字段：error=SMS_SECURITY_VERIFY_REQUIRED│
│            redirect_url=<校验链接>          │
│                                            │
│  提示用户：                                 │
...
7. **安全验证（20010）处理**：当 send-sms 返回 `error=SMS_SECURITY_VERIFY_REQUIRED` 时，必须引导用户点击 `redirect_url` 完成安全验证；用户确认完成后，**重新调用 send-sms**（无需用户再次输入手机号）；安全验证后的短信由后端自动触发，用户直接输入收到的验证码即可
